...
public class MotionProfileFollower {
    ...
    protected MotionProfilePoint mLatestActualState;
    protected MotionProfilePoint mInitialState;
    ...
    /**
     * Create a new MotionProfileFollower.
     *
     ...
     */
    public MotionProfileFollower(double kp, double ki, double kv, double kffv, double kffa,
                           double ks) {
        resetProfile();
    ...
    public void resetProfile() {
        mTotalError = 0.0;
        mInitialState = MotionProfilePoint.kInvalidState;
        mLatestActualState = MotionProfilePoint.kInvalidState;
     ...
     */
    public void setGoalAndConstraints(MotionProfileGoal goal, MaxVel_MaxAcc constraints) {
    ...
    public void setConstraints(MaxVel_MaxAcc constraints) {
        setGoalAndConstraints(mGoal, constraints);
    }

    public MotionProfilePoint getSetpoint() {
        return (mLatestSetpoint == null ? MotionProfilePoint.kInvalidState : mLatestSetpoint.motion_state);
    }
     ...
     */
    public synchronized double update(MotionProfilePoint latest_state, double t) {
        mLatestActualState = latest_state;
        MotionProfilePoint prev_state = latest_state;
        if (mLatestSetpoint != null) {
            prev_state = mLatestSetpoint.motion_state;
        } else {
            mInitialState = prev_state;
        }
        final double dt = Math.max(0.0, t - prev_state.getTime());
        mLatestSetpoint = mSetpointGenerator.getSetpoint(mConstraints, mGoal, prev_state, t);

        // Update error.
        mLatestPosError = mLatestSetpoint.motion_state.getPosition() - latest_state.getPosition();
        mLatestVelError = mLatestSetpoint.motion_state.getVelocity() - latest_state.getVelocity();

        // Calculate the feedforward and proportional terms.
        double output = mKp * mLatestPosError + mKv * mLatestVelError + mKffv * mLatestSetpoint.motion_state.getVelocity()
                + (Double.isNaN(mLatestSetpoint.motion_state.getAcceleration()) ? 0.0 : mKffa * mLatestSetpoint.motion_state.getAcceleration());

        if (!Utility.epsilonEquals(output, 0.0)) {
    ...
    public boolean onTarget() {
        ...
        // the finish line.
        final double goal_to_start = mGoal.getPosition() - mInitialState.getPosition();
        final double goal_to_actual = mGoal.getPosition() - mLatestActualState.getPosition();
