...
import frc.lib.motion_profile.MaxVel_MaxAcc;
import frc.lib.motion_profile.MotionProfileGoal;
import frc.lib.motion_profile.MotionProfileGoal.CompletionBehavior;
import frc.lib.motion_profile.MotionProfilePoint;
import frc.lib.motion_profile.MotionProfileFollower;
 ...
 * MotionProfileFollower to generate a profile (displacement and velocity) command.
 */
public class PathFollower {
    ...
    MotionProfileFollower mVelocityController;
    ...
    public PathFollower(Path path, boolean reversed, Parameters parameters) {
        ...
        mVelocityController.setConstraints(
                new MaxVel_MaxAcc(parameters.profile_max_abs_vel, parameters.profile_max_abs_acc));
    ...
    public synchronized Twist2D update(double t, Pose2D pose, double displacement, double velocity) {
        if (!mSteeringController.isFinished()) {
            ...
            mVelocityController.setGoalAndConstraints(
                    ...
                    new MaxVel_MaxAcc(Math.min(mMaxProfileVel, steering_command.max_velocity),
                            mMaxProfileAcc));
        ...
        final double velocity_command = mVelocityController.update(new MotionProfilePoint(t, displacement, velocity, 0.0), t);
        ...
        if (!Double.isNaN(curvature) && Math.abs(curvature) < kReallyBigNumber) {
            ...
            final double abs_velocity_setpoint = Math.abs(mVelocityController.getSetpoint().getVelocity());
        ...
        mDebugOutput.profile_displacement = mVelocityController.getSetpoint().getPosition();
        mDebugOutput.profile_velocity = mVelocityController.getSetpoint().getVelocity();
